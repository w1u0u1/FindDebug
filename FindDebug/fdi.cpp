...
GETDEBUGINFOCALLBACK GetDebugInfo_CB = NULL;

void zero_str(char* ptr)
{
...
void CleanDebugDirectoryEntries(LPBYTE pImageBase, PIMAGE_DEBUG_DIRECTORY pDebugDir, DWORD DebugDirSize, bool clean, bool cleanPath)
{
	...
	for (int i = 1; i <= NumEntries; i++, pDebugDir++)
	{
		...
		memset((char*)pDebugInfo, 0, pDebugDir->SizeOfData);
	}
}
...
void DumpDebugDirectoryEntries(LPBYTE pImageBase, PIMAGE_DEBUG_DIRECTORY pDebugDir, DWORD DebugDirSize)
{
	...
	for (int i = 1; i <= NumEntries; i++, pDebugDir++)
	{
		...
		if (GetDebugInfo_CB)
			GetDebugInfo_CB(pImageBase, pDebugDir);
	}
}
...
void DumpMiscDebugInfo(LPBYTE pDebugInfo, DWORD DebugInfoSize)
{
	...
	_tprintf(_T("Data type: %u  Length: %u  Format: %s \n"), pMiscInfo->DataType, pMiscInfo->Length, pMiscInfo->Unicode ? _T("Unicode") : _T("ANSI"));
